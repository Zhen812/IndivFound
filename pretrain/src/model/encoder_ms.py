...
class MultiMAE(nn.Module):
    ...
    def __init__(self, opt, norm_layer=nn.LayerNorm):
        ...
        print('an Individual Characterization MAE')
        ...
        self.norm_video = norm_layer(opt['encoder']['embed_dim'])
        self.norm_eeg = norm_layer(opt['encoder']['embed_dim'])
        self.norm_ecg = norm_layer(opt['encoder']['embed_dim'])
        self.norm_eog = norm_layer(opt['encoder']['embed_dim'])
        self.norm_emg = norm_layer(opt['encoder']['embed_dim'])
        self.norm_gsr = norm_layer(opt['encoder']['embed_dim'])
    ...
    def forward_decoder(self, x, mask_video, ids_restore_video,
        ...
        x_video = self.decoder_pred_video(x_video)
        x_eeg = self.decoder_pred_eeg(x_eeg)
        x_ecg = self.decoder_pred_ecg(x_ecg)
        x_eog = self.decoder_pred_eog(x_eog)
        x_emg = self.decoder_pred_emg(x_emg)
        x_gsr = self.decoder_pred_gsr(x_gsr)
