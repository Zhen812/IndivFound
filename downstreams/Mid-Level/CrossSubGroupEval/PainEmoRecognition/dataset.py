            ...
            for filename in filenames:
                ...
                for clip in clips:
                    ...
                    if contain_idx != -1:
                        ...
                        tmp.extend([os.path.join(pre_dir, '%s%s.npy' % (
                            '_'.join(new_clip_name_list[:-1]), new_clip_name_list[-1])) for pre_dir in pre_dirs])
                        age_num = int(subj.split('_')[-1])
                        age_label = 0 if age_num < 50 else 1
                        sex_indicator = subj.split('_')[1]
                        sex_label = 0 if sex_indicator.lower() == 'm' else 1
                        ...
                        if (self.mode == 'train' and label_choice[self.filter_choice[0]] == self.filter_choice[1]) or (
                                self.mode == 'test' and label_choice[self.filter_choice[0]] == 1 - self.filter_choice[
                            1]):
