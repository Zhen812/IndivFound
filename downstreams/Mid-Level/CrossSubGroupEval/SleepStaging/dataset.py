...
class IC_Dataset(Dataset):
    ...
    def __init__(self, opt, mode, filter_choice, dataset='sleep-cassette'):
        super(IC_Dataset, self).__init__()
        self.mode = mode
        self.opt = opt
        self.dataset = dataset
        self.data_dir = os.path.join(opt['data_dir'], dataset)
        anno_path = opt['anno_path']
        anno_df = pd.read_excel(anno_path)
        subject2anno = {}
        for i in range(len(anno_df)):
            subjectID = int(anno_df.iloc[i]['subject'])
            age = int(anno_df.iloc[i]['age'])
            sex = int(anno_df.iloc[i]['sex (F=1)'])
            age = 0 if age < 50 else 1
            sex = 0 if sex == 2 else 1
            subject2anno[subjectID] = {"age": age, 'sex': sex}
        self.subject2anno = subject2anno
    ...
    def _obtain_data(self):
        ...
        for subject in subjects:
            subjectID = int(subject[-2:])
            age = self.subject2anno[subjectID]['age']
            sex = self.subject2anno[subjectID]['sex']
