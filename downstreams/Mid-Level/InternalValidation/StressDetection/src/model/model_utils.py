...
class PatchEmbed_1d(nn.Module):
    def __init__(self, in_channels, tubelet_size, length, embed_dim=768):
        super().__init__()
        num_tokens = length // tubelet_size
        self.modal_length = length
        self.tubelet_size = tubelet_size
        self.num_patches = num_tokens

        self.proj = nn.Conv1d(in_channels, embed_dim, kernel_size=tubelet_size, stride=tubelet_size)
        self._init_weights()
        self.norm = nn.LayerNorm(embed_dim, eps=1e-3)
    ...
    def forward(self, x):
        # x: (N,L,D)
        if x.size()[1] < self.tubelet_size:
            return torch.zeros(size=(x.size()[0], 0, self.proj.out_channels)).cuda()
...
class SplitAttention(nn.Module):
    def __init__(self, dim: int,
        ...
        self.cross_attention = nn.ModuleList([CrossAttention(dim, num_heads, qkv_bias, qk_norm, attn_drop, proj_drop, norm_layer) for _ in range(self.num_splits)])
        self.norm_2 = norm_layer(dim)

    def forward(self, x, split_lengths):
        B, L, D = x.shape
        splits = torch.split(x, split_lengths, dim=1)
        self_attend_outputs = [s + self_attn(self_norm(s)) for self_norm, self_attn, s in
        ...
        for i, s in enumerate(self_attend_outputs):
            if len(self_attend_outputs) > 1:
                ...
                cross_attend_output = self.cross_attention[i].forward(x_q=s, x_kv=others_concatenated)
...
#         super(Block_u, self).__init__()
#         self.norm1 = norm_layer(dim)
#         self.norm1_video = norm_layer(dim)
#         self.norm1_eeg = norm_layer(dim)
#         self.norm1_ecg = norm_layer(dim)
#         self.norm1_eog = norm_layer(dim)
#         self.norm1_emg = norm_layer(dim)
#         self.norm1_gsr = norm_layer(dim)
#         self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
#         self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
#         self.norm2 = norm_layer(dim)
#         self.norm2_video = norm_layer(dim)
#         self.norm2_eeg = norm_layer(dim)
#         self.norm2_ecg = norm_layer(dim)
#         self.norm2_eog = norm_layer(dim)
#         self.norm2_emg = norm_layer(dim)
#         self.norm2_gsr = norm_layer(dim)
#         mlp_hidden_dim = int(dim * mlp_ratio)
#         self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_video = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_eeg = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_ecg = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_eog = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_emg = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#         self.mlp_gsr = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#
...
#             x = x + self.drop_path(self.mlp(self.norm2(x)))
#         elif modality == "video":
#             x = x + self.drop_path(self.attn(self.norm1_video(x)))
#             x = x + self.drop_path(self.mlp_video(self.norm2_video(x)))
#         elif modality == "eeg":
#             x = x + self.drop_path(self.attn(self.norm1_eeg(x)))
#             x = x + self.drop_path(self.mlp_eeg(self.norm2_eeg(x)))
#         elif modality == "ecg":
#             x = x + self.drop_path(self.attn(self.norm1_ecg(x)))
#             x = x + self.drop_path(self.mlp_ecg(self.norm2_ecg(x)))
#         elif modality == "eog":
#             x = x + self.drop_path(self.attn(self.norm1_eog(x)))
#             x = x + self.drop_path(self.mlp_eog(self.norm2_eog(x)))
#         elif modality == "emg":
#             x = x + self.drop_path(self.attn(self.norm1_emg(x)))
#             x = x + self.drop_path(self.mlp_emg(self.norm2_emg(x)))
#         elif modality == "gsr":
#             x = x + self.drop_path(self.attn(self.norm1_gsr(x)))
#             x = x + self.drop_path(self.mlp_gsr(self.norm2_gsr(x)))
#         return x
...
def patchify_2d(imgs, patch_size):
    """
    :param imgs: [N, channel_num,H,W]
    :param c: channel_num
    :param patch_size:
    :return: [N,L,D]
    """
    N, channel_num, H, W = imgs.size()
    x = imgs.reshape(N, channel_num, H // patch_size, patch_size, W // patch_size, patch_size)
    x = torch.einsum('nchpwq->nhwpqc', x)
    x = x.reshape(N, (H * W) // (patch_size * patch_size), patch_size * patch_size * channel_num)
    # x = nn.LayerNorm(patch_size * patch_size * channel_num, eps=1e-3)(x)
    return x


def unpatchify_2d(seq, height, width, patch_size):
    """
    :param seq: [N,L,D]
    :param height:
    :param width:
    :param patch_size:
    :return: [N,channel_num,H,W]
    """
    N, L, D = seq.size()
    channel_num = D // (patch_size * patch_size)
    x = seq.reshape(N, height // patch_size, width // patch_size, patch_size, patch_size, channel_num)
    x = torch.einsum('nhwpqc->nchpwq', x)
    x = x.reshape(N, channel_num, height, width)
    return x
