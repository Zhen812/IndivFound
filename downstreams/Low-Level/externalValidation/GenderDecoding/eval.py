...
class IC_Dataset_AutonomicAging(Dataset):
    def __init__(self, opt, label="Age_group"):
        super(IC_Dataset_AutonomicAging, self).__init__()
        self.opt = opt
        self.label = label
    ...
    def _obtain_data(self):
        ...
        final_subjects = subject2anno.keys()
        data_dir = self.opt['data_dir']
        filenames = os.listdir(data_dir)
        subj2files = {subj: [] for subj in final_subjects}
        file_paths, labels = [], []
        for filename in filenames:
            subjectID = filename.split('_')[0]
            if subjectID in final_subjects:
                subj2files[subjectID].append(os.path.join(data_dir, filename))

        for k, v in subj2files.items():
            v.sort()
            v_len = len(v)
            file_paths.extend(v)
            labels.extend([subject2anno[k]] * v_len)
...
def external_val_AutonomicAging2ECGID_ours():
...
def external_val_ECGID2AutonomicAging():
    with open("./AutonomicAging_config.yaml", encoding="UTF-8") as f:
        opt = yaml.safe_load(f)
    # load data
    test_dataset = IC_Dataset_AutonomicAging(opt, label=args.label)
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=opt['batch_size'],
                                              shuffle=False,
                                              num_workers=opt['num_workers'])
...
if __name__ == '__main__':
    print('[External Validation] on [Gender Decoding task], from [Autonomic-Aging] Datset to [ECGID] Datset')
    print('--' * 15)
    external_val_AutonomicAging2ECGID_ours()
    print('[External Validation] on [Gender Decoding task], from [ECGID] Datset to [Autonomic-Aging] Datset')
    print('--' * 15)
    external_val_ECGID2AutonomicAging()
