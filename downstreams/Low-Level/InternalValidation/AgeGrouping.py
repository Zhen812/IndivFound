...
class IC_Dataset(Dataset):
    def __init__(self, opt, mode="train", label="Age_group", dataAmount=1., filter_choice=None, testFold=None):
    ...
    def _obtain_data(self):
        ...
        for k, v in subj2files.items():
            ...
            v_len = len(v)
            if self.mode == "train":
                file_paths.extend(v[:int(self.dataAmount * v_len)])
                labels.extend([subject2anno[k]] * int(self.dataAmount * v_len))
            else:
                file_paths.extend(v)
...
def evaluate():
    with open(os.path.join(os.getcwd(), "config.yaml"), encoding="UTF-8") as f:
        opt = yaml.safe_load(f)

    found_model_ckpts = opt['found_model_ckpts']['ageGrouping']
    cls_model_ckpts = opt['cls_model_ckpts']['ageGrouping']
    folds = list(range(5))
...
if __name__ == '__main__':
    print('[Internal Validation] on [Age Grouping] Task with [Autonomic-Aging] Dataset')
    print('--' * 15)
    evaluate()
