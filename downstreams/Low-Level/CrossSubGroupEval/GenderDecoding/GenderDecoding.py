...
def cross_subgroup_val_Y2O_mae():
    ...
    for foldID in folds:
        # load data
        test_dataset = IC_Dataset(opt, mode='test', label=args.label, filter_choice=('Age_group', 0))
...
def cross_subgroup_val_O2Y_mae():
    ...
    for foldID in folds:
        # load data
        test_dataset = IC_Dataset(opt, mode='test',label=args.label, filter_choice=('Age_group', 1))
        test_loader = torch.utils.data.DataLoader(test_dataset,
                                                  batch_size=opt['batch_size'],
                                                  shuffle=False,
                                                  num_workers=opt['num_workers'])
...
if __name__ == '__main__':
    print('[Cross Sub-Group Validation] on [Gnder Decoding] Task with [Autonomic Aging Dataset]')
